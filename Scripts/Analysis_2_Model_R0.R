...
for(cc in 1:length(testing_countriesList)){
  ...
  training_ready_sub_vsurf_result = select(training_ready_sub2, c("R0",colnames(training_ready_sub2[,results.vsurf$varselect.pred])))
  glimpse(training_ready_sub_vsurf_result)
  
  # testing dataframe with reduced number of variables
  testing_ready_sub_vsurf_result = select(testing_ready, c("R0",colnames(training_ready_sub2[,results.vsurf$varselect.pred])))
  ...
  for(i in breaker:nrow(testing_ready_pred)){
    for(l in 1:nLags){
      if(incidence_flag==T && death_flag==F){
        if(c(paste0(sprintf("R0_Lag_%02d", l))) %in% colnames(testing_ready_pred)){
          testing_ready_pred[i,c(paste0(sprintf("R0_Lag_%02d", l)))] <- testing_ready_pred[i-l,c(paste0(outcomeVariable))]
        }
      }
    }
    if(incidence_flag==T && death_flag==F){
  ...
  pdf(paste0("finalPlot_",testing_countries[1],"_R0.pdf"),width = 24, height = 24)
