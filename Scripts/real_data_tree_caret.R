...
if(death_flag==F){
  ...
  if(incidence_flag==T){
    ...
    set.seed(825)
    gbm.mod <- train(confirmed_cum_per_million ~ .,
    ...
    # Chris' original fit
    # fit <- rpart(confirmed_cum ~ .,
    #              data = training_ready_sub2,
    #              method="anova", #"anova", "poisson", "class" or "exp"
    #              control=rpart.control(minsplit=2, cp=0.0001))
    # # rpart.plot(fit, main="Tree")
    # fitrf <- randomForest(confirmed_cum ~ .,
    #                       data = training_ready_sub2,
    #                       importance = TRUE,
    #                       na.action = na.omit)
  }
  ...
  if(incidence_flag==T){
    ...
    # Chris' original fit
    # fit <- rpart(death_cum_per_million ~ .,
    #              data = training_ready_sub2,
    #              method="anova", #"anova", "poisson", "class" or "exp"
    #              control=rpart.control(minsplit=2, cp=0.0001))
    # fitrf <- randomForest(death_cum_per_million ~ .,
    #                       data = training_ready_sub2,
    #                       importance = TRUE,
    #                       na.action = na.omit)
  }else{
