...
cc=1
  ...
  for(i in breaker:nrow(testing_ready_pred)){
    for(l in 1:nLags){
      if(incidence_flag==T && death_flag==F){
        if(c(paste0(sprintf("confirmed_cum_per_million_lag_%02d", l))) %in% colnames(testing_ready_pred)){
          testing_ready_pred[i,c(paste0(sprintf("confirmed_cum_per_million_lag_%02d", l)))] <- testing_ready_pred[i-l,c(paste0(outcomeVariable))]
        }
      }
    }
    if(incidence_flag==T && death_flag==F){
  ...
  pdf(paste0("./Output_CaseIncidence/finalPlot_",testing_countries[1],"_CaseIncidence.pdf"),width = 24, height = 24)
