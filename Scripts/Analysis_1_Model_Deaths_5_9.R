...
for(cc in 1:length(testing_countriesList)){
  ...
  D4 <- data_clean1$date[which.max(data_clean1$death_cum_per_million)]
  D3 <- data_clean1$date[which.min(abs(data_clean1$death_cum_per_million - data_clean1$death_cum_per_million[which(data_clean1$date == D4)]*2/3))]
  D2 <- data_clean1$date[which.min(abs(data_clean1$death_cum_per_million - data_clean1$death_cum_per_million[which(data_clean1$date == D4)]*1/3))]
  D1 <- data_clean1$date[which.min(abs(data_clean1$death_cum_per_million - data_clean1$death_cum_per_million[which(data_clean1$date == D4)]*1/10))]
  print(D1)
  print(D2)
  print(D3)
  print(D4)
  ...
  # for(timeChop in c(D4)){
    ...
    training_ready_sub2 <- training_ready %>%
      ...
      select(-contains("confirmed_cum")) %>%
      select(-contains("MalePercent")) %>%
      select(-contains("FemalePercent")) %>%
      select(-c(death_cum, Percent_house_Multi_generation,Percent_house_Three_generation,Percent_house_Skip_generation,Num_Tests_cum)) %>%
    ...
    if(timeChop == D4){
      ...
      save(nmj_used, VSURF_thres_keepers, VSURF_interp_keepers, VSURF_pred_keepers, results.vsurf, results.vsurf.OG, file = paste0("./InputData/",testing_country,"_VSURFkeepers_daths.Rdata"))
    }
    VSURF_thres_keepers <- NULL
    VSURF_interp_keepers <- NULL
    VSURF_pred_keepers <- NULL
    # VSURF_thres_keepers <- "test"
    # VSURF_interp_keepers <- "test"
    # VSURF_pred_keepers <- "test"
    load(paste0("./InputData/",testing_country,"_VSURFkeepers_daths.Rdata"))
    ...
    if(length(VSURF_pred_keepers) > 1){
      ...
      training_ready_sub_vsurf_result_varImp = select(training_ready_sub2, c(outcomeVariable,VSURF_interp_keepers))
    }else if(length(VSURF_pred_keepers) <= 1 && length(VSURF_interp_keepers) <= 1 && length(VSURF_thres_keepers) > 1){
      ...
      training_ready_sub_vsurf_result_varImp <- training_ready_sub2
    }
    ...
    if(length(VSURF_pred_keepers) > 1){
      ...
      testing_ready_sub_vsurf_result <- testing_ready_sub2
    }
    ...
    for(i in breaker:nrow(testing_ready_pred)){
      for(l in 1:nLags){
          if(c(paste0(sprintf("death_cum_per_million_lag_%02d", l))) %in% colnames(testing_ready_pred)){
            testing_ready_pred[i,c(paste0(sprintf("death_cum_per_million_lag_%02d", l)))] <- testing_ready_pred[i-l,c(paste0(outcomeVariable))]
          }
      }
    ...
    plot1Data <- merge(plot1Data, hindsight[,c("date","death_cum_per_million")], by="date", all.x=T, all.y=T)
    ...
    pdf(paste0("./Output_DeathIncidence/finalPlot_",testing_country,"_",dateName,"_DeathIncidence.pdf"),width = 24, height = 24)
    ...
    save(plot1, plot_predict, plot2, plot3, plot4, best_model,  file = paste0("./COVID-19_Shiny_Web_App/Inputs/",testing_country,"_",dateName, "_DeathIncidence_Data.RData") )
